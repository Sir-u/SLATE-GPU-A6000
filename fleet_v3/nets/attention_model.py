...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_spatial:
            spatial_node_dim = 2 + num_veh  # (x,y) + demand/capacity per vehicle
    ...
    def _init_embed(self, input):
        ...
        if self.is_hcvrp:
            # demand_norm: (B, N, num_veh) = demand / capacity_per_vehicle
            ...
            return torch.cat(  # [batch_size, graph_size+1, embed_dim]
                (
                    ...
                    self.init_embed(torch.cat((  # [batch_size, graph_size, embed_dim]
                        ...
                        demand_norm  # [batch_size, graph_size, num_veh]
                    ), -1))
                ),
                1
            )
    ...
    def select_veh(self, input, state, sequences, embeddings, obj, veh_list, tour_1, tour_2, tour_3):
        ...
        if sequences:
            ...
            mean_tour = torch.zeros([batch_size, 3 * embed_dim], device=embeddings.device, dtype=embeddings.dtype)
    ...
    def _build_raw_features(self, input):
        ...
        Expects: input['depot'] (B,2), input['loc'] (B,N,2), input['demand'] (B,N), input['capacity'] (B,V)
        """
        depot_xy = input['depot'][:, None, :]  # (B,1,2)
        demand_norm = input['demand'][..., None] / input['capacity'][:, None, :]  # (B,N,V)
    ...
    def _get_parallel_step_context(self, embeddings, state, veh, from_depot=False):
        ...
        if self.is_hcvrp:
            ...
            if from_depot:
                ...
                cap = torch.as_tensor(self.problem.VEHICLE_CAPACITY, device=device, dtype=dtype)  # (V,)
                rem = cap[None, :, None] - torch.zeros_like(state.used_capacity[:, :, None])      # (B,V,1)
                return torch.cat(
                    (
                ...
                cap = torch.as_tensor(self.problem.VEHICLE_CAPACITY, device=device, dtype=dtype)  # (V,)
                cap_b = cap.index_select(0, veh)  # (B,)
                rem_cap = cap_b - state.used_capacity[torch.arange(batch_size, device=device), veh]  # (B,)
