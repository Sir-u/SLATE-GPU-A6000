...
def _estimate_encoder_flops_from_batch(model, batch) -> int:
    # Shapes
    B = int(batch['loc'].shape[0])          # batch size
    N = int(batch['loc'].shape[1])          # customers
    T = N + 1                                # depot + customers

    # Model hyperparams (fallbacks if missing)
    E = int(getattr(model, 'embedding_dim', 128))
    H = int(getattr(model, 'n_heads', 8))
    L = int(getattr(model, 'n_encode_layers', 3))
    hidden = int(getattr(model, 'feed_forward_hidden', 512))
    use_spatial = bool(getattr(model, 'use_spatial', False))
    W = int(getattr(model, 'spatial_window', 32) or 32)  # window for spatial encoder
    d = max(1, E // max(1, H))
    node_dim = 3  # (x, y, demand) for depot/customer features

    # Count MACs (multiply-adds), convert to FLOPs by *2
    macs = 0
    # Initial embedding
    macs += B * T * node_dim * E
    ...
    for _ in range(L):
        ...
        if use_spatial:
            macs += B * H * T * W * d   # QK^T (local window)
            macs += B * H * T * W * d   # Attn*V
        else:
            macs += B * H * T * T * d   # QK^T (full)
            macs += B * H * T * T * d   # Attn*V
        # Output projection
        macs += B * T * E * E
        # FFN two-linears E->hidden->E
        macs += B * T * E * hidden + B * T * hidden * E
...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    model.to(device)
    model.eval()
    # Reset peak VRAM before evaluation on this device
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        # FLOPs estimate (encoder-only, first batch once)
        if getattr(opts, 'flops_estimate', False) and not did_flops:
            try:
                est = _estimate_encoder_flops_from_batch(model, batch)
                per_sample = est / max(1, int(batch['loc'].shape[0]))
                print(f"[FLOPs] encoder approx per batch: {_format_flops(est)} "
                      f"({ _format_flops(per_sample) } per sample, "
                      f"L={getattr(model,'n_encode_layers',3)}, "
                      f"H={getattr(model,'n_heads',8)}, "
                      f"E={getattr(model,'embedding_dim',128)}, "
                      f"N={int(batch['loc'].shape[1])})")
        ...
        # Optional VRAM print per batch
        if getattr(opts, 'vram', False) and torch.cuda.is_available() and device.type == "cuda":
            alloc, reserved, peak = _gpu_mem_stats(device)
            print(f"[VRAM:{device}] alloc={_to_mib(alloc):.1f} MiB, reserved={_to_mib(reserved):.1f} MiB, peak={_to_mib(peak):.1f} MiB")

    # Final peak summary
    if getattr(opts, 'vram', False) and torch.cuda.is_available() and device.type == "cuda":
        _, _, peak = _gpu_mem_stats(device)
        print(f"[VRAM:{device}] peak during eval: {_to_mib(peak):.1f} MiB")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vram', action='store_true', help='Print VRAM usage per batch and peak')
    parser.add_argument('--flops_estimate', action='store_true', help='Print approximate encoder FLOPs (first batch only)')
