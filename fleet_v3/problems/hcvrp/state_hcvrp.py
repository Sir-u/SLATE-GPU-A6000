...
StateHCVRPBase = namedtuple('StateHCVRP', [
    'coords',
    'demand',
    'ids',
    'veh',
...
class StateHCVRP(StateHCVRPBase):
    # Fixed input
    ...
    def get_mask(self, veh):
        ...
        # Device-safe capacity check per selected vehicle
        dev = self.demand.device
        used_sel = self.used_capacity[torch.arange(batch_size, device=dev), veh][:, None, None]
        cap_vec = torch.as_tensor(self.VEHICLE_CAPACITY, device=dev, dtype=self.demand.dtype)
        cap_sel = cap_vec[veh][:, None, None]
        exceeds_cap = (self.demand[self.ids, 1:] + used_sel) > cap_sel
